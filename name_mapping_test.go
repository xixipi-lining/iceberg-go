...
func TestUpdateNameMapping(t *testing.T) {
	...
	t.Run("rename and add new fields", func(t *testing.T) {
		...
		adds := map[int][]iceberg.NestedField{
			-1: {
				{ID: 2, Name: "foo", Type: &iceberg.StringType{}},
			},
		}
		...
		expected := iceberg.NameMapping{
			{FieldID: makeID(1), Names: []string{"new_foo"}},
			{FieldID: makeID(2), Names: []string{"foo"}},
		}
